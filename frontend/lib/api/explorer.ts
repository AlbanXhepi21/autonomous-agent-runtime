...
import type { ArtifactMetadata, DatabaseSchemaSummary, DatabaseTable, TableDescription } from "@/types/api";

export type { ArtifactMetadata as Artifact, DatabaseTable, TableDescription };

export const explorerApi = {
  artifacts: (runId?: string) => request<ArtifactMetadata[]>(`/artifacts${runId ? `?run_id=${encodeURIComponent(runId)}` : ""}`),
