...
function isDatabaseTable(value: unknown): value is DatabaseTable {
  return Boolean(
    value &&
    typeof value === "object" &&
    typeof (value as DatabaseTable).name === "string" &&
    (value as DatabaseTable).name.trim(),
  );
}

export function DatabaseExplorer() {
  const [tables, setTables] = useState<DatabaseTable[]>([]);
  const [selected, setSelected] = useState<TableDescription | null>(null);
  ...
  useEffect(() => {
    void schemaApi
      ...
      .catch(() => setError("Database schema is unavailable."));
  }, []);
  useEffect(() => {
    if (selected) panel.current?.scrollIntoView?.({ behavior: "smooth", block: "start" });
  }, [selected]);
  ...
  return (
    <aside className="explorer-panel" ref={panel}>
      <h2>
        Database <small>{tables.length} tables</small>
      </h2>
      <input
        ...
        {selected && (
          <section className="table-detail">
            ...
            <div className="table-detail-body">
              <h4>
                Columns <small>{selected.columns.length}</small>
              </h4>
              <ul>
                {selected.columns.map((column) => (
                  <li key={column.name}>
                    ...
                    {column.primary_key && <span className="column-flag pk">PK</span>}
                    {!column.nullable && <span className="column-flag">required</span>}
                  </li>
                ))}
              </ul>
              {selected.foreign_keys.length > 0 && (
                <>
                  ...
                  <ul className="relationships">
                    {selected.foreign_keys.map((key, index) => (
                      <li key={index}>
                        {key.source_column} → {key.target_table}.{key.target_column}
                      </li>
                    ))}
                  </ul>
                </>
