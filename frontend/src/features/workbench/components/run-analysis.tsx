...
function eventFilter(event: PublicRunEvent, filter: Filter) {
  ...
  if (filter === "tools")
    return (
      ...
      event.type.startsWith("schema.") ||
      event.type.startsWith("python.") ||
      event.type.startsWith("delegation.")
    );
  if (filter === "artifacts")
    return (
      event.type.includes("artifact") ||
      event.type.includes("chart") ||
...
function eventText(event: PublicRunEvent) {
  ...
  if (event.type === "skill.loaded")
    return `Data analysis skill loaded${typeof data.skill === "string" ? `: ${data.skill}` : ""}`;
  if (event.type === "schema.table_described")
    return `Inspected ${(Array.isArray(data.tables) ? data.tables : []).join(", ") || "table"}`;
  ...
  if (event.type === "sql.query_rejected")
    return `${queryLabel(event)} rejected by SQL safety policy`;
  ...
  if (
    ...
    event.type === "chart.created" ||
    event.type === "report.created"
  )
    return "Artifact created";
  if (event.type === "delegation.started")
    return `Delegated analysis${typeof data.agent_name === "string" ? ` → ${data.agent_name}` : ""}`;
  ...
  if (event.type === "security.policy_evaluated")
    return data.decision === "deny"
      ? "Tool denied by capability policy"
...
function queryLabel(event: PublicRunEvent) {
  return `Query #${typeof event.data.query_id === "string" ? event.data.query_id.replace(/^query_?0*/, "") || event.data.query_id : "?"}`;
}
function toolName(data: Record<string, unknown>) {
  return typeof data.tool_name === "string" ? data.tool_name.replaceAll("_", " ") : "Tool";
}
function isFailure(event: PublicRunEvent) {
  return (
    event.type.includes("failed") ||
    event.type.includes("rejected") ||
    (event.type === "security.policy_evaluated" && event.data.decision === "deny")
  );
}
  ...
  return (
    <dl className="run-metrics">
      <div>
        <dt>Duration</dt>
        <dd>
          {metric(metrics.total_duration_ms === null ? null : `${metrics.total_duration_ms}ms`)}
        </dd>
      </div>
      <div>
        <dt>Iterations</dt>
        <dd>{metrics.iterations}</dd>
      </div>
      <div>
        <dt>SQL queries</dt>
        <dd>{metrics.database_query_count}</dd>
      </div>
      <div>
        <dt>Tool calls</dt>
        <dd>{metrics.tool_calls}</dd>
      </div>
      <div>
        <dt>Tokens</dt>
        <dd>{metric(metrics.total_tokens)}</dd>
      </div>
      <div>
        ...
        <dd
          title={
            metrics.estimated_cost === null
              ? "Configure OpenAI per-million token prices on the backend to calculate this."
              : undefined
          }
        >
          {metrics.estimated_cost === null
...
export function RunAnalysis({
  ...
  const visible = useMemo(
    () => events.filter((event) => eventFilter(event, filter)),
    [events, filter],
  );
  return (
    <section className="run-analysis">
      ...
      {open && (
        <div className="analysis-body">
          ...
          {loading && <p className="trace-empty">Loading structured trace…</p>}
          {!loading && visible.length === 0 && (
            <p className="trace-empty">No public trace events are available for this run.</p>
          )}
          <ol className="trace-events">
            {visible.map((event) => (
              <li key={event.id} className={isFailure(event) ? "failure" : ""}>
                <span className="trace-icon">
                  {isFailure(event) ? "⚠" : event.type.endsWith("started") ? "●" : "✓"}
                </span>
                <div>
                  <strong>{eventText(event)}</strong>
                  {event.type.startsWith("sql.") && (
                    <div className="query-details">
                      <span>
                        Status:{" "}
                        {event.type === "sql.query_completed"
                          ? "Completed"
                          : event.type === "sql.query_failed"
                            ? "Failed"
                            : event.type === "sql.query_rejected"
                              ? "Rejected"
                              : "Running"}
                      </span>
                      ...
                      {typeof event.data.row_count === "number" && (
                        <span>Rows: {event.data.row_count}</span>
                      )}
                      {Array.isArray(event.data.referenced_tables) &&
                        event.data.referenced_tables.length > 0 && (
                          <span>Tables: {event.data.referenced_tables.join(", ")}</span>
                        )}
                      <details>
                        <summary>SQL</summary>
                        <p>
                          {typeof event.data.sql === "string" ? (
                            <code>{event.data.sql}</code>
                          ) : (
                            "SQL is not retained or exposed for this run."
                          )}
                        </p>
                      </details>
                    </div>
                  ...
                  {typeof event.data.error === "string" && (
                    <p className="trace-reason">Reason: {event.data.error}</p>
                  )}
                </div>
              </li>
            ))}
