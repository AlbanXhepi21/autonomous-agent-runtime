...
vi.mock("@/lib/api/analytics", () => ({ analyticsApi: { createRun: vi.fn(), getRun: vi.fn(), connect: vi.fn() } }));
vi.mock("@/lib/api/conversations", () => ({ conversationsApi: { create: vi.fn(), list: vi.fn(), get: vi.fn(), rename: vi.fn(), remove: vi.fn() } }));
vi.mock("@/lib/api/approvals", () => ({ approvalsApi: { list: vi.fn(), approve: vi.fn(), reject: vi.fn() } }));

describe("Workbench", () => {
  const connect = vi.mocked(analyticsApi.connect);
  beforeEach(() => { vi.resetAllMocks(); vi.mocked(conversationsApi.list).mockResolvedValue({ items: [], total: 0, limit: 30, offset: 0 }); vi.mocked(approvalsApi.list).mockResolvedValue([]); });
  ...
  it("submits a message and shows running status", async () => {
    ...
    render(<Workbench />);
    fireEvent.change(screen.getByLabelText("Ask about your data"), { target: { value: "Why did revenue fall?" } });
  ...
  it("shows a backend error without adding an assistant response", async () => {
    vi.mocked(analyticsApi.createRun).mockRejectedValue(new ApiError("The analyst backend is unavailable."));
    render(<Workbench />);
    fireEvent.change(screen.getByLabelText("Ask about your data"), { target: { value: "Analyze revenue" } });
    fireEvent.click(screen.getByRole("button", { name: "Analyze" }));
    expect(await screen.findByRole("alert")).toHaveTextContent("The analyst backend is unavailable.");
  });
  ...
  it("explains when a run is waiting for a protected-action approval", async () => {
    ...
    handler?.({ id: "event-approval", run_id: "run-approval", type: "run.completed", timestamp: "", data: {} });

    expect(await screen.findByRole("region", { name: "Approval required" })).toHaveTextContent("write_file");
  ...
  it("creates a clean conversation", async () => {
    vi.mocked(conversationsApi.create).mockResolvedValue({ id: "new", title: "New conversation", created_at: "", updated_at: "" });
    render(<Workbench />);
  ...
  it("paginates the sidebar and deletes a conversation from its options menu", async () => {
    const firstPage = Array.from({ length: 8 }, (_, index) => ({ id: `c-${index}`, title: `Conversation ${index}`, created_at: "", updated_at: "" }));
    const secondPage = Array.from({ length: 2 }, (_, index) => ({ id: `c-${index + 8}`, title: `Conversation ${index + 8}`, created_at: "", updated_at: "" }));
    vi.mocked(conversationsApi.list).mockResolvedValueOnce({ items: firstPage, total: 10, limit: 8, offset: 0 }).mockResolvedValueOnce({ items: secondPage, total: 10, limit: 8, offset: 8 });
    ...
    fireEvent.click(screen.getByRole("button", { name: "Conversation options for Conversation 0" }));
