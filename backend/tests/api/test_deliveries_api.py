...
error, and that history is listable afterward.
"""
...
class FakeDeliveryStore:
    ...
    async def list(self, *, artifact_id=None, status=None):
...
def test_create_delivery_for_a_ready_artifact_succeeds() -> None:
    ...
    response = client.post("/api/v1/deliveries", json={"artifact_id": "artifact-1", "channel": "link", "destination": "n/a"})
...
def test_create_delivery_for_an_unconfigured_channel_is_refused() -> None:
    ...
    response = client.post("/api/v1/deliveries", json={"artifact_id": "artifact-1", "channel": "email", "destination": "a@b.com"})
...
def test_list_deliveries_reflects_prior_attempts() -> None:
    _app, client = _client(_artifact())
    client.post("/api/v1/deliveries", json={"artifact_id": "artifact-1", "channel": "link", "destination": "n/a"})

    response = client.get("/api/v1/deliveries", params={"artifact_id": "artifact-1"})
