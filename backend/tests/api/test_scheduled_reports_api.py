...
def _saved_report(**overrides) -> SavedReportDefinition:
    ...
    fields = dict(
        id=uuid4(), workspace_id="default", owner=None, name="Weekly Revenue", description=None,
...
def _client(saved_reports: FakeSavedReportStore, schedules: FakeScheduledReportStore) -> TestClient:
    application = FastAPI()
...
def test_create_computes_next_run_at_and_returns_the_schedule() -> None:
    ...
    response = client.post("/api/v1/reports/scheduled", json=_create_body(str(saved.id)))
...
def test_create_is_404_for_an_unknown_saved_report() -> None:
    ...
    response = client.post("/api/v1/reports/scheduled", json=_create_body(str(uuid4())))
...
def test_create_is_404_across_workspaces() -> None:
    saved = _saved_report(workspace_id="workspace-a")
    client = _client(FakeSavedReportStore(reports={saved.id: saved}), FakeScheduledReportStore())

    response = client.post(
        "/api/v1/reports/scheduled",
        json={**_create_body(str(saved.id)), "workspace_id": "workspace-b"},
    )
...
def test_delivery_channel_without_destination_is_rejected() -> None:
    ...
    response = client.post(
        "/api/v1/reports/scheduled", json={**_create_body(str(saved.id)), "delivery_channel": "webhook"},
    )
...
def test_list_and_get_round_trip() -> None:
    saved = _saved_report()
    client = _client(FakeSavedReportStore(reports={saved.id: saved}), FakeScheduledReportStore())
    created = client.post("/api/v1/reports/scheduled", json=_create_body(str(saved.id))).json()

    listed = client.get("/api/v1/reports/scheduled").json()
    fetched = client.get(f"/api/v1/reports/scheduled/{created['id']}").json()
...
def test_get_is_404_for_an_unknown_id() -> None:
    ...
    response = client.get(f"/api/v1/reports/scheduled/{uuid4()}")
...
def test_patch_can_disable_a_schedule() -> None:
    ...
    response = client.patch(f"/api/v1/reports/scheduled/{created['id']}", json={"enabled": False})
...
def test_patch_changing_the_schedule_recomputes_next_run_at() -> None:
    ...
    response = client.patch(
        f"/api/v1/reports/scheduled/{created['id']}",
...
def test_patch_is_404_for_an_unknown_id() -> None:
    ...
    response = client.patch(f"/api/v1/reports/scheduled/{uuid4()}", json={"enabled": False})
...
def test_patch_with_a_mismatched_delivery_pair_is_rejected() -> None:
    ...
    response = client.patch(
        f"/api/v1/reports/scheduled/{created['id']}", json={"delivery_destination": "https://example.com/hook"},
    )
