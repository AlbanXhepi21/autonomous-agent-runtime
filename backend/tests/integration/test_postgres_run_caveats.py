...
async def _finished_run(database: Database, caveats: list[str] | None):
    ...
    conversation, _, run = await store.create_run(
        workspace_id=WORKSPACE_ID, conversation_id=None, message="Investigate refunds", run_id=f"caveat-test-{uuid4()}"
    )
    from datetime import UTC, datetime

    await store.finish_run(
        workspace_id=WORKSPACE_ID, run_id=run.id, status="completed", completed_at=datetime.now(UTC), metrics=None,
...
async def test_stated_caveats_are_read_back_by_a_later_store(database) -> None:
    ...
    try:
        ...
        reloaded = await PostgresConversationStore(database).get_run(workspace_id=WORKSPACE_ID, run_id=run_id)
