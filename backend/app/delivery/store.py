...
def _to_domain(row: DeliveryRow) -> DeliveryRecord:
    return DeliveryRecord(
        id=row.id, artifact_id=row.artifact_id, channel=row.channel, destination=row.destination,
        status=row.status, attempt_count=row.attempt_count, last_attempt_at=row.last_attempt_at,
        provider_metadata=dict(row.provider_metadata or {}), failure_reason=row.failure_reason,
...
class DeliveryStore:
    ...
    async def create(self, *, artifact_id: str, channel: DeliveryChannel, destination: str) -> DeliveryRecord:
        raise NotImplementedError

    async def get(self, delivery_id: UUID) -> DeliveryRecord | None:
        raise NotImplementedError

    async def list(self, *, artifact_id: str | None = None, status: DeliveryStatus | None = None) -> list[DeliveryRecord]:
        raise NotImplementedError

    async def record_attempt(
        self, *, delivery_id: UUID, status: DeliveryStatus, provider_metadata: dict[str, Any],
...
class PostgresDeliveryStore(DeliveryStore):
    ...
    async def create(self, *, artifact_id: str, channel: DeliveryChannel, destination: str) -> DeliveryRecord:
        ...
        row = DeliveryRow(
            ...
            status="pending", attempt_count=0, last_attempt_at=None, provider_metadata=None,
            failure_reason=None, created_at=stamp, updated_at=stamp,
        )
