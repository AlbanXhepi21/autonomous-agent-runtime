...
@router.post("", response_model=ConversationResponse, status_code=201, dependencies=[Depends(require_csrf)])
async def create_conversation(
    request: ConversationCreateRequest,
    context: TenantContext = Depends(require_permission(Permission.RUN_ANALYSES)),
    store: ConversationStore = Depends(get_conversation_store),
) -> ConversationResponse:
    return _conversation(await store.create_conversation(workspace_id=context.workspace.id, title=request.title))
...
async def list_conversations(
    ...
    items, total = await store.list_conversations(workspace_id=context.workspace.id, limit=limit, offset=offset)
...
async def get_conversation(
    conversation_id: UUID, message_limit: int = Query(default=100, ge=1, le=200),
    message_offset: int = Query(default=0, ge=0),
    context: TenantContext = Depends(require_permission(Permission.READ_TENANT_RESOURCES)),
    store: ConversationStore = Depends(get_conversation_store),
) -> ConversationDetailResponse:
    conversation = await store.get_conversation(workspace_id=context.workspace.id, conversation_id=conversation_id)
    ...
    messages, total = await store.list_messages(workspace_id=context.workspace.id, conversation_id=conversation_id, limit=message_limit, offset=message_offset)
    runs = await store.list_runs(workspace_id=context.workspace.id, conversation_id=conversation_id)
...
@router.patch("/{conversation_id}", response_model=ConversationResponse, dependencies=[Depends(require_csrf)])
async def update_conversation(
    conversation_id: UUID, request: ConversationTitleRequest,
    context: TenantContext = Depends(require_permission(Permission.RUN_ANALYSES)),
    store: ConversationStore = Depends(get_conversation_store),
) -> ConversationResponse:
    conversation = await store.update_title(workspace_id=context.workspace.id, conversation_id=conversation_id, title=request.title)
    if conversation is None: raise HTTPException(status_code=404, detail={"code": "unknown_conversation", "message": "Conversation not found."})
    return _conversation(conversation)


@router.delete("/{conversation_id}", status_code=204, dependencies=[Depends(require_csrf)])
async def delete_conversation(
    conversation_id: UUID,
    context: TenantContext = Depends(require_permission(Permission.RUN_ANALYSES)),
    store: ConversationStore = Depends(get_conversation_store),
) -> Response:
    if not await store.delete_conversation(workspace_id=context.workspace.id, conversation_id=conversation_id):
