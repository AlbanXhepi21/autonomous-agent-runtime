...
from app.api.dependencies import require_developer_mode, require_permission
...
router = APIRouter(prefix="/api/v1/workspaces/{workspace_id}/memory", tags=["memory-inspector"])
...
async def list_memory(
    memory_type: MemoryType | None = None, run_id: str | None = None, session_id: str | None = None,
    created_after: datetime | None = Query(default=None), _: None = Depends(require_developer_mode),
    context: TenantContext = Depends(require_permission(Permission.READ_TENANT_RESOURCES)),
    store: MemoryStore = Depends(get_memory_store),
):
    memories = await store.list_memories(
        workspace_id=context.workspace.id, memory_type=memory_type, run_id=run_id, session_id=session_id,
    )
...
async def get_memory(
    memory_id: UUID, _: None = Depends(require_developer_mode),
    context: TenantContext = Depends(require_permission(Permission.READ_TENANT_RESOURCES)),
    store: MemoryStore = Depends(get_memory_store),
):
    memory = await store.get(workspace_id=context.workspace.id, memory_id=memory_id)
