...
class GenerateReportTool(Tool):
    ...
    async def execute_for_run(self, *, run_id: str | None, workspace_id: str | None = None, **arguments: Any) -> dict[str, object]:
        ...
        if not workspace_id: raise ToolInputError("Report generation requires a tenant workspace.")
        ...
        artifacts = []
        try:
            artifacts.append(await self._register(run_id, workspace_id, directory, "report.md", render_markdown(report), "report", "text/markdown", report))
            artifacts.append(await self._register(run_id, workspace_id, directory, "supporting_metrics.json", report.model_dump_json(indent=2), "report_data", "application/json", report))
            for index, dataset_id in enumerate(report.dataset_ids_for_csv, start=1):
                ...
                artifacts.append(await self._register_path(run_id, workspace_id, path, "csv_extract", "text/csv", report))
    ...
    async def _register(self, run_id: str, workspace_id: str, directory: Path, name: str, text: str, artifact_type: str, media_type: str, report: AnalyticalReport) -> dict[str, object]:
        path = directory / name; path.write_text(text, encoding="utf-8")
        return await self._register_path(run_id, workspace_id, path, artifact_type, media_type, report)
    async def _register_path(self, run_id: str, workspace_id: str, path: Path, artifact_type: str, media_type: str, report: AnalyticalReport) -> dict[str, object]:
        artifact = await self._artifacts.register(workspace_id=UUID(workspace_id), run_id=run_id, source_path=path.relative_to(self._workspace.root).as_posix(), artifact_type=artifact_type, media_type=media_type, metadata={"report_type": report.report_type.value, "time_period": report.time_period, "source_query_ids": report.source_query_ids})
